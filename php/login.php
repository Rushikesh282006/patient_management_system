...
if(isset($_SESSION["loggedin"]) && $_SESSION["loggedin"] === true){
    $role = $_SESSION["role"];
    echo json_encode(["success" => true, "redirect" => "{$role}_dashboard.php"]);
    exit;
}
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
                    ...
                    if($stmt->fetch()){
                        if(password_verify($password, $hashed_password) || $password === $hashed_password){
                            ...
                            echo json_encode([
                                ...
                                "redirect" => "{$db_role}_dashboard.php"
                            ]);
                            exit();
                        } else{
    ...
    // If we reach here, there was an error. Return JSON error
    echo json_encode([
        "success" => false,
        "message" => $login_err ?: $username_err ?: $password_err ?: $role_err
    ]);
    exit();
}
?>
